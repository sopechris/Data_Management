...
# Funtion to run sequential feature selection algorithm to find the features that maximize performance of a model.
    ...
    def draw_sfs(self, model, scoring=None, strategy='k-fold', cv=10, iters=1, shuffle=False, direction='backward'):
        ...
        # Initialize the Sequential Feature Selector
        if direction == 'backward':
            sfs = SequentialFeatureSelector(model, k_features=1, forward=False, scoring=scoring, cv=cv_model, n_jobs=-1)
        elif direction == 'forward':
            sfs = SequentialFeatureSelector(model, k_features=len(self.feature_names)-1, forward=True, scoring=scoring,
                    cv=cv_model, n_jobs=-1)
        else:
            raise ValueError('Direction must be one of forward or backward.')
        ...
        # Color feature names according to clusters
        ...
        max_idx = np.argmax(info[:, 1])
        max_metric = info[max_idx, 1]
        max_std = info[max_idx, 2]
