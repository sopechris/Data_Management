...
class FeatureList:
    ...
    def sequential_feature_selection(self, model, top, scoring=None, strategy='k-fold', cv=10, iters=1,
        ...
        # Create train set from input feature object.
        ...
        # Create forward or backward sequential feature selection object depending on input parameters.
        if direction == 'backward':
            sfs = SequentialFeatureSelector(model, k_features=top, forward=False, floating=floating,
                                            scoring=scoring, cv=cv_model, n_jobs=cpu_num)
        elif direction == 'forward':
            sfs = SequentialFeatureSelector(model, k_features=top, forward=True,
                                            floating=floating, scoring=scoring, cv=cv_model, n_jobs=cpu_num)
        else:
            raise ValueError('Direction must be one of forward or backward.')

        # Fit sfs model to input train set.
        sfs = sfs.fit(X, y)
        
        # Get the names of the selected and rejected features.
    ...
    # Draw results of sequential feature search algorithm.
    def draw_sfs(self, model, scoring=None, strategy='k-fold',
                cv=10, iters=1, shuffle=False, direction='backward'):
        ...
        # Create forward or backward sequential feature selection object depending on input parameters.
        if direction == 'backward':
            sfs = SequentialFeatureSelector(model, k_features=1, forward=False, scoring=scoring, cv=cv_model, n_jobs=cpu_num)
        elif direction == 'forward':
            sfs = SequentialFeatureSelector(model, k_features=len(self.feature_names)-1, forward=True, scoring=scoring,
                    cv=cv_model, n_jobs=cpu_num)
        else:
            raise ValueError('Direction must be one of forward or backward.')
